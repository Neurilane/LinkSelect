...
import {
  ...
  StyleSheet, Modal, Alert, Clipboard
} from 'react-native';
import { collection, onSnapshot, query, where, orderBy } from 'firebase/firestore';
import { db } from '../config/firebase';

export default function ClienteFaturasScreen({ usuario }) {
  ...
  function copiarPIX() {
    Clipboard.setString(faturaSelecionada?.pixCode || '00020126360014BR.GOV.BCB.PIX');
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitulo}>Minhas Faturas</Text>
      </View>
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.card} onPress={() => abrirSegundaVia(item)}>
            <View style={styles.cardInfo}>
              <Text style={styles.cardNome}>Fatura {item.vencimento}</Text>
              <Text style={styles.cardSub}>R$ {item.valor} • Toque para 2ª via</Text>
            </View>
